  ...
  <footer class="bg-morado-scorpios text-white">
    <div class="max-w-full">
      <div class="px-6 sm:px-12 py-4">
        <h1 class="text-2xl sm:text-4xl-plus font-bold">Deider Yofret Ruiz Acosta</h1>
      </div>
      <div>
        <svg xmlns="http://www.w3.org/2000/svg" class="w-full h-16">
          <defs>
            <pattern
              id="a"
              width="100"
      ...
      >
        <div class="space-y-3">
          ...
          <div class="space-y-3 text-base sm:text-xl">
            <div
              v-for="aboutMe in datos.aboutMe"
              :key="aboutMe"
              class="font-semibold flex items-center justify-start space-x-3 sm:space-x-5"
            >
              <img
                :src="aboutMe.srcImage"
                :alt="aboutMe.altImage"
        ...
        <div class="space-y-3 text-sm sm:text-base">
          <div>
            <h3 class="text-lg sm:text-2xl font-bold">{{ datos.footer.disponibilidad }}</h3>
            <ul class="list-disc list-inside">
              <li v-for="trabajo in datos.footer.trabajos" :key="trabajo">
                {{ trabajo }}
              </li>
            </ul>
          </div>
          <a
            ...
            target="_blank"
            class="block w-full sm:max-w-fit font-bold text-sm sm:text-base text-center py-2 px-3 border-3 border-white text-white hover:ring-3 hover:ring-white hover:ring-offset-3 hover:ring-offset-morado-scorpios hover:bg-white hover:text-morado-scorpios transition transform ease-in-out duration-300 flex items-center justify-center space-x-2"
          >
