...
import PopupForLanguage from '@/components/PopupForLanguage.vue'
import PopupForMenu from '@/components/PopupForMenu.vue'
...
<template>
  ...
  >
    <div class="w-full">
      <div
        class="absolute top-0 left-0 h-1 bg-gradient-to-r from-gris-scorpios to-white transition-all duration-200"
      ...
      <div class="w-full flex flex-wrap items-center justify-between mx-auto py-4 px-6 sm:px-12">
        ...
        <div class="flex items-center justify-end text-sm sm:text-base space-x-3 md:space-x-0">
          <div class="relative">
            <dropdown-button
              :icono="datos.language.bandera"
              :texto="datos.language.lengua"
              :open="modal === 'idioma'"
              @toggle="openModal('idioma')"
            ></dropdown-button>
            <!-- Dropdown -->
            <scale-fade-transition>
              <div ref="dropdownRef" v-show="modal === 'idioma'">
                <popup-for-language @select="setLanguage"></popup-for-language>
              </div>
            </scale-fade-transition>
          </div>
          <div class="relative md:hidden">
            <dropdown-button
              ...
              :texto="'Menú'"
              :open="modal === 'menu'"
              @toggle="openModal('menu')"
            ></dropdown-button>
            <!-- Dropdown -->
            <scale-fade-transition>
              <div ref="dropdownRef" v-show="modal === 'menu'">
                <popup-for-menu :secciones="datos.secciones"></popup-for-menu>
              </div>
            </scale-fade-transition>
          </div>
        </div>
